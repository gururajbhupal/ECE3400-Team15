...
void rf() {
  ...
  // Store data and set as explored
  set_explored(data);
  maze[x][y] = data;
...
void push_unvisited() {
  if (!check_right() && !is_explored(maze[right.x][right.y])) {
    stack.push(right);
  }
  if (!check_left() && !is_explored(maze[left.x][left.y])) {
    stack.push(left);
  }
  if (!check_front() && !is_explored(maze[front.x][front.y])) {
    stack.push(front);
  }
