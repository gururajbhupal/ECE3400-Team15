...
/* Searches for and builds a path to given coordinate from the current coordinate*/
void find_path(Coordinate v) {
  ...
  while (next.x != v.x || next.y != v.y) {
    ...
    /*Choose next coordinate based on heading, and known wall information at each coordinate*/
    switch (h) {
...
void traverse_path(QueueList <Coordinate> path) {
  ...
  while (!path.isEmpty()) {
    ...
    if (p.x == front.x && p.y == front.y && is_in_bounds(front)) {
    ...
    else if (p.x == left.x && p.y == left.y && is_in_bounds(left)) {
    ...
    else if (p.x == right.x && p.y == right.y && is_in_bounds(right)) {
